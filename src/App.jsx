...
export default function App() {
  ...
  // Загружаем рекорд при старте приложения
  useEffect(() => {
    tg?.ready();
    tg?.expand();
  ...
  const sendRecord = async () => {
    ...
    // ВСЁ ОБНОВЛЯЕМ СРАЗУ И НАВСЕГДА
    setHighScore(score);
    localStorage.setItem('dotlock_hs', score); // ←←← ГАРАНТИРОВАННО СОХРАНЯЕТСЯ
    await loadGlobalTop();
    ...
    const timer = setInterval(() => {
      setTimeLeft(t => {
        if (t <= 1) {
          ...
          if (score > highScore) sendRecord(); // ←←← ОТПРАВКА ТОЛЬКО ПРИ НОВОМ РЕКОРДЕ
          return 0;
        }
        return t - 1;
      });
